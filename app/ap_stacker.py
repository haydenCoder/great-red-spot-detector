...
def _lk_refine(ref: np.ndarray, img: np.ndarray, ay: float, ax: float,
    ...
    for _ in range(iters):
        ys = yy - cy
        xs = xx - cx
        warped = map_coordinates(img, [ys, xs], order=3, mode="nearest")
        gy = 0.5 * (map_coordinates(img, [ys + 1, xs], order=3, mode="nearest")
                    - map_coordinates(img, [ys - 1, xs], order=3, mode="nearest"))
        gx = 0.5 * (map_coordinates(img, [ys, xs + 1], order=3, mode="nearest")
                    - map_coordinates(img, [ys, xs - 1], order=3, mode="nearest"))
