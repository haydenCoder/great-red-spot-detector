...
Full write-up: tests/test_deterioration_regressions.py (before/after pinned).
...
[`tests/test_real_photo_audit.py`](tests/test_real_photo_audit.py).
...
### Added
- `tools/real_photo_validate.py` — disk / softness / band / RGB-vs-mono /
  redness-vs-dark split checks on untimed real frames (no invented System III
  truth).
...
The deep audit (results in `runs/`, regenerated by `tools/per_method_audit.py`) found ~5% of 2.40″ very-blurry
...
- `tools/seeing_floor_stress.py` — reproducible very-blurry catastrophic-rate check.
...
[`tests/test_planetary_stacker.py`](tests/test_planetary_stacker.py).
...
- pinned by `tests/test_planetary_stacker.py`.
...
[`tests/test_jupiter_zonal.py`](tests/test_jupiter_zonal.py).
...
- `app/cli.py` — added `zonal-stack` and `zonal-derotate`
  subcommands.
- regression suite: `tests/test_jupiter_zonal.py`.
...
frames. Full write-up: `tools/per_method_audit.py` + `tests/test_science_p0_fixes.py`
and the regression suite under `tests/`.
...
- consolidated work log: `CHANGELOG.md` plus `tests/` regressions.
...
| `README.md` + `docs/ESSAY.md` | Background, geometry, ephemeris, validation summary. |
...
measurability floor. Full write-up: `tools/per_method_audit.py` (regenerable results).
...
### Added
- `tools/per_method_audit.py` — resumable large-N audit harness (resolution ×
  seeing × seed, lean, planted-centre truth).
- consolidated deep-audit write-up: `tools/per_method_audit.py`.
...
[`tools/accuracy_campaign.py`](tools/accuracy_campaign.py) + `tools/per_method_audit.py`.
...
- consolidated audit: `tools/accuracy_campaign.py` + `tools/per_method_audit.py`.
...
See `tests/test_geometry_smoke.py` (and the smoke suite).
...
See `tests/test_geometry_100.py` / `tests/test_resolution_seeing_100.py`.
